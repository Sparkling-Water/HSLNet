...
from model.base import vit_utils as utils
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.allow_mod:
            ...
            self.prompt_tokens = nn.Parameter(
                torch.zeros(ncls * divider, opt['num_prompt'], embed_dim))  # [bank_size, G, embed_dim]
        ...
        if pretrained == "":
            ...
            if logger is not None:
                ...
                logger.info(' ' * 5 + f'==> {backbone} initialized from {pretrained}')
    ...
    def forward_original(self, x):
        ...
        b, n, c = x_0.shape
        hh = int(math.sqrt(n))
        x_0 = x_0.view(b, hh, hh, c).permute(0, 3, 1, 2).contiguous()
        ...
        # high-level features, [B,C,H,W]
        x = x[:, self.num_tokens:, :]
        b, n, c = x.shape
        hh = int(math.sqrt(n))
        x = x.view(b, hh, hh, c).permute(0, 3, 1, 2).contiguous()

        return dict(out=x, out_0=x_0)
    ...
    def forward(self, x):
        ...
        return dict(out=x, tokens=tokens, out_0=x_0)
...
def _load_weights_pth(model: VisionTransformer, checkpoint_path):
    ...
    _logger.info(' ' * 5 + f"==> {counter} parameters loaded.")
