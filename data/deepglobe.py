...
class DatasetDeepglobe(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, num=1200):
        ...
        # class name
        self.categories = ['agriculture_land','urban_land','rangeland','forest_land','water','barren_land']
        # class id
    ...
    def build_img_metadata_classwise(self):
        ...
        for idx, cat in enumerate(self.categories):
            img_paths = sorted([path for path in glob.glob('%s/*' % os.path.join(self.base_path, str(idx+1), 'test', 'origin'))])
            for img_path in img_paths:
                if os.path.basename(img_path).split('.')[1] == 'jpg':
