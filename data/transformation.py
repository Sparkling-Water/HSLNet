...
class Compose(object):
    ...
    def __call__(self, image, label, spixel_label, **kwargs):
        if self.processer == 'cv2':
            image = np.float32(image)
            label = np.uint8(label)
        # spixel_label = np.float32(spixel_label)
        for t in self.transforms:
            image, label, spixel_label, kwargs = t(image, label, spixel_label, proc=self.processer, **kwargs)
            
        return image, label, spixel_label, kwargs
...
class Resize(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        if proc == 'pil':
            w, h = img.size
        else:
            h, w, _ = img.shape
        ...
        if self.do_mask:
            spixel_mask_list = []
            for i in range(spixel_mask.shape[-1]):
                spixel_mask_list.append(cv2.resize(spixel_mask[:,:,i], (int(ow/8), int(oh/8)), interpolation=cv2.INTER_NEAREST))
            spixel_mask = np.stack(spixel_mask_list, axis=-1)
        
        if 'weights' in kwargs:
            kwargs['weights'] = cv2.resize(kwargs['weights'], (ow, oh), interpolation=cv2.INTER_LINEAR)

        return img, mask, spixel_mask, kwargs
...
class ToTensor(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        ...
        spixel_mask = torch.from_numpy(np.array(spixel_mask))
...
class RandomCrop(object):
    ...
    def __init__(self, height, width, check=False, prop=0.85, max_iter=30, center=True, pad_type='reflect'):
        ...
        self.center = center
        if pad_type == 'reflect':
            self.pad_kwargs_image = self.pad_kwargs_mask = self.pad_kwargs_weight = self.pad_kwargs_spixel_mask = {
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        ...
        if w != tw or h != th:
            if w < tw or h < th:
                ...
                if spixel_mask is not None:
                    spixel_mask_list = []
                    for i in range(spixel_mask.shape[-1]):
                        spixel_mask_list.append(cv2.resize(spixel_mask[:,:,i], (tw, th), interpolation=cv2.INTER_NEAREST))
                    spixel_mask = np.stack(spixel_mask_list, axis=-1)

            raw_img = img
            raw_mask = mask
            raw_spixel_mask = spixel_mask
            raw_weights = weights
            if proc == 'pil':
                w, h = img.size
            else:
            ...
            if spixel_mask is not None:
                spixel_mask = raw_spixel_mask[y1:y1 + th, x1:x1 + tw, :]
        ...
        if spixel_mask is not None:
            spixel_mask_list = []
            for i in range(spixel_mask.shape[-1]):
                spixel_mask_list.append(cv2.resize(spixel_mask[:,:,i], (int(tw/8), int(th/8)), interpolation=cv2.INTER_NEAREST))
            spixel_mask = np.stack(spixel_mask_list, axis=-1)    
...
class RandomResize(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        ...
        if 'weights' in kwargs:
            kwargs['weights'] = cv2.resize(kwargs['weights'], (tw, th), interpolation=cv2.INTER_LINEAR)
...
class RandomSizedCrop(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        img, mask, spixel_mask, kwargs = self.resize(img, mask, spixel_mask, proc=proc, **kwargs)
        img, mask, spixel_mask, kwargs = self.crop(img, mask, spixel_mask, proc=proc, **kwargs)
        return img, mask, spixel_mask, kwargs
...
class RandomHorizontallyFlip(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        if random.random() < self.p:
            ...
            if spixel_mask is not None:
                for i in range(spixel_mask.shape[-1]):
                    spixel_mask[:, :, i] = np.ascontiguousarray(np.flip(spixel_mask[:, :, i], axis=1))
...
class RandomVerticallyFlip(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        if random.random() < self.p:
            ...
            if spixel_mask is not None:
                for i in range(spixel_mask.shape[-1]):
                    spixel_mask[:, :, i] = np.ascontiguousarray(np.flip(spixel_mask[:, :, i], axis=0))
...
class RandomRotate(object):
    ...
    def __call__(self, img, mask, spixel_mask, proc='pil', **kwargs):
        if random.random() < self.p:
            ...
            spixel_mask_list = []
            for i in range(spixel_mask.shape[-1]):
                spixel_mask_list.append(cv2.warpAffine(spixel_mask[:, :, i], matrix, (w, h), flags=cv2.INTER_NEAREST, **self.pad_kwargs_spixel_mask))
            spixel_mask = np.stack(spixel_mask_list, axis=-1)
            if proc == 'pil':
